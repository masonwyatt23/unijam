...
type HeldCandidate = { candidate?: { title?: string; artists?: string[]; provider?: "spotify" | "apple_music"; providerRecordingId?: string; providerUrl?: string } };
type Resolution =
...
function ContributionForm({ roomId, participantId, canContribute, offerMembership, provider, onProviderChange, onStaged }: { roomId: string; participantId: string; canContribute: boolean; offerMembership: boolean; provider: Provider; onProviderChange: (provider: Provider) => void; onStaged: () => void }) {
  // A harmless draft may survive the passkey/provider round-trip, but it must
  // not leak to a different participant who later opens this room in the tab.
  const draftKey = `unijam.song-draft.${roomId}.${participantId}`;
  ...
  const roomReturnTo = `/room/${roomId}`;
  const spotifyConnectionReturnTo = `/connections/spotify?returnTo=${encodeURIComponent(roomReturnTo)}`;
  const spotifyMembershipHref = `/host/sign-in?returnTo=${encodeURIComponent(spotifyConnectionReturnTo)}`;
  ...
  function updateInput(value: string) {
    ...
    } catch { /* The field still works without browser storage. */ }
  }
  ...
  async function selectHeld(candidate: HeldCandidate) {
    if (!candidate.candidate?.title || !candidate.candidate.providerUrl || !candidate.candidate.provider || !candidate.candidate.providerRecordingId) return;
    setStatus("working"); setMessage("");
    try {
      const resolutionResponse = await fetch(`/api/v1/rooms/${encodeURIComponent(roomId)}/resolve`, {
        method: "POST", credentials: "include", headers: { "content-type": "application/json" },
        body: JSON.stringify({ input: input.trim(), provider, selection: candidate.candidate.providerRecordingId }),
      });
      const resolutionBody = await resolutionResponse.json() as ApiEnvelope<Resolution>;
      if (!resolutionResponse.ok || resolutionBody.error || !resolutionBody.data || resolutionBody.data.status !== "matched") {
        throw new Error(resolutionBody.error?.message ?? "This version is no longer a current match. Search again.");
      }
      await stageResolution(resolutionBody.data);
    } catch (cause) {
      setStatus("error");
  ...
  async function contribute(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (resolutionBody.data.status === "hold") {
        setStatus("hold");
        const actionable = resolutionBody.data.candidates.some(({ candidate }) => candidate?.providerRecordingId && candidate.providerUrl);
        setMessage(resolutionBody.data.reasons.map((reason) => holdLabels[reason] ?? "We need a little more detail").join(". ") + (actionable ? ". Listen to the choices and pick the exact recording." : ". No US recording can be selected from these results."));
...
export default function LiveRoomPage() {
  ...
  const [listeningPreference, setListeningPreference] = useState<Provider>(() => {
    ...
    try {
      const saved = sessionStorage.getItem(`unijam.listening-preference.${roomId}`);
