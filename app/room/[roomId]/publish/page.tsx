...
import { ErrorPanel, LoadingPanel, PageHeader, ProductShell, ProviderBrand, RecordingArtwork, SegmentedControl, StatusBanner, useRoomState } from "@/app/components/product";
...
export default function PublishPage() {
  ...
  const occurrenceByRecording = useMemo(() => new Map(room.data?.snapshot.occurrences.map((item) => [item.recordingId, item]) ?? []), [room.data?.snapshot.occurrences]);
  ...
  return <ProductShell roomId={roomId} displayName={actor.nickname}><PageHeader eyebrow="PUBLISH" title={`Publish room ${roomId}`} description="Create one new private playlist at a time. Each service succeeds, fails, retries, or cancels independently." backHref={`/room/${roomId}/recap`} />
    ...
    {preview ? <section className="publish-preview"><div className="preview-title"><div><p className="eyebrow">IMMUTABLE DESTINATION</p><h2>{preview.destination.name}</h2><p>New private {preview.provider === "spotify" ? "Spotify" : "Apple Music"} playlist · room sequence {preview.roomRevision}</p><small>{preview.destination.description}</small></div><span className="immutable"><Check /> Fingerprint locked</span></div><ol className="compact-tracklist publish-tracklist">{preview.items.map((item) => { const occurrence = occurrenceByRecording.get(item.canonicalRecordingId); const display = occurrence?.display; return <li key={item.itemKey}><span>{String(item.position + 1).padStart(2, "0")}</span><RecordingArtwork display={display} title={occurrence?.title ?? "Matched recording"} className="publish-track-artwork" size={58} /><div className="publish-track-copy"><strong>{occurrence?.title ?? "Matched recording"}</strong><small>{display?.artists.join(", ") || "Artist unavailable"}{display?.album ? ` · ${display.album}` : ""}</small><span>{durationLabel(display?.durationMs)}{display?.explicit ? `${display?.durationMs ? " · " : ""}Explicit` : ""}</span></div></li>; })}</ol>
