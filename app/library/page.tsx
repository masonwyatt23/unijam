  ...
  return <ProductShell roomId={roomId || undefined} displayName={host.data.displayName}>
    ...
    <section className="library-deck" aria-labelledby="library-provider-title">
      ...
      <div className={`provider-shelf provider-shelf-${provider}`}>
        ...
        {!failure && !loading && page?.items.length === 0 ? <div className="library-inline-state"><Library /><div><strong>{submittedQuery ? "No songs found" : "No saved songs yet"}</strong><p>{submittedQuery ? "Try a title, artist, or album with fewer words." : `Save songs in ${name}, then refresh this shelf.`}</p><button className="button button-quiet" onClick={() => void load()}>Refresh</button></div></div> : null}
        {page && page.items.length > 0 ? <ol className="library-track-list" aria-label={`${name} ${submittedQuery ? "search results" : "saved songs"}`}>{page.items.map((track, index) => <li key={`${track.providerRecordingId}-${index}`}><a className="library-art-link" href={track.providerUrl} target="_blank" rel="noreferrer" aria-label={`Open ${track.title} on ${name}`}><TrackArtwork track={track} /></a><div className="library-track-copy"><a href={track.providerUrl} target="_blank" rel="noreferrer"><strong>{track.title}</strong></a><span>{track.artists.join(", ")}</span><small>{track.album ?? "Album unavailable"}{track.explicit ? " · Explicit" : ""}</small></div><span className="library-duration">{durationLabel(track.durationMs)}</span><div className="library-track-actions">{roomId ? <button className="button button-primary" disabled={addingId !== null} onClick={() => void addToRoom(track)}>{addingId === track.providerRecordingId ? <><LoaderCircle className="spinning" /> Adding…</> : <><Plus /> Add to room</>}</button> : null}{track.provider === "spotify" ? <ProviderBrand provider="spotify" compact background="light" purpose="attribution" href={track.providerUrl} label={`Open ${track.title} on Spotify`} /> : <ProviderBrand provider="apple-music" compact variant="listen-badge" background="light" purpose="attribution" href={track.providerUrl} label={`Listen to ${track.title} on Apple Music`} />}</div></li>)}</ol> : null}
