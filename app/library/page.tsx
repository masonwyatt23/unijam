  ...
  return <ProductShell roomId={roomId || undefined} displayName={host.data.displayName}>
    ...
    <section className="library-deck" aria-labelledby="library-provider-title">
      ...
      <div className={`provider-shelf provider-shelf-${provider}`}>
        ...
        {page && page.items.length > 0 ? <ol className="library-track-list" aria-label={`${name} ${submittedQuery ? "search results" : "saved songs"}`}>{page.items.map((track, index) => <li key={`${track.providerRecordingId}-${index}`}><a className="library-art-link" href={track.providerUrl} target="_blank" rel="noreferrer" aria-label={`Open ${track.title} on ${name}`}><TrackArtwork track={track} /></a><div className="library-track-copy"><a href={track.providerUrl} target="_blank" rel="noreferrer"><strong>{track.title}</strong></a><span>{track.artists.join(", ")}</span><small>{track.album ?? "Album unavailable"}{track.explicit ? " · Explicit" : ""}</small></div><span className="library-duration">{durationLabel(track.durationMs)}</span><div className="library-track-actions">{roomId ? <button className="button button-primary" disabled={addingId !== null} onClick={() => void addToRoom(track)}>{addingId === track.providerRecordingId ? <><LoaderCircle className="spinning" /> Adding…</> : <><Plus /> Add to room</>}</button> : null}<a href={track.providerUrl} target="_blank" rel="noreferrer">Open on {providerName(provider)}</a></div></li>)}</ol> : null}
