...
type SpotifyBrand = { provider: "spotify"; variant?: "full-logo"; background: "light" | "dark"; purpose: "attribution" | "connect" | "handoff" | "published"; href?: string; label?: string };
type AppleMusicBrand = { provider: "apple-music"; variant: "listen-badge"; background: "light" | "dark"; purpose: "handoff" | "published"; href: string; label?: string };
...
export function ProviderBrand(props: ProviderBrandProps) {
  ...
  const content = <img src={image} alt={alt} width={props.provider === "spotify" ? 96 : 111} height={props.provider === "spotify" ? 40 : 33} />;
  return props.href ? <a className={className} href={props.href} target="_blank" rel="noreferrer">{content}<span className="sr-only"> (opens in a new tab)</span></a> : <span className={className}>{content}</span>;
}
...
export function LivingSetlist({ snapshot, guest, onRefresh }: { snapshot: RoomSnapshot; guest: boolean; onRefresh: () => void }) {
