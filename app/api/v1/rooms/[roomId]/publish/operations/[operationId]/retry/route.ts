...
export async function POST(request: Request, context: Context): Promise<Response> {
  ...
  if (!host) return apiError("RECENT_PASSKEY_REQUIRED", "Confirm a passkey before retrying publishing", 403);
  return connectorJsonRequest(env as ConnectorProxyEnv, "/v1/publish/retry", { accountId: host.account_id, operationId });
}
