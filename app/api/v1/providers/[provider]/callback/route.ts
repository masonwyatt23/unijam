...
import { connectorJsonRequest, normalizeProvider, publicAppOrigin, type ConnectorProxyEnv } from "@/lib/server/connector-proxy";
...
async function callback(request: Request, context: Context): Promise<Response> {
  ...
  try {
    ...
    const response = await connectorJsonRequest(env as ConnectorProxyEnv, "/v1/oauth/spotify/callback", {
      code, state, callbackUrl: `${origin}/api/v1/providers/spotify/callback`,
    });
